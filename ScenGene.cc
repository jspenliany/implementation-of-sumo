...
ScenGene::file_SPATHtcl(){
	...
	for(int si=1; si < node_num+1; si++){
		for(int ei=1; ei < node_num+1; ei++){
			if(ei == si) continue;
			InitshortestPath();
			int path[node_num];
			shortestPath((si-1)/node_colc+1,(si-1)%node_colc+1,(ei-1)/node_colc+1,(ei-1)%node_colc+1,path);
			strtmp<<"set shortpath"<<si<<"TO"<<ei<<"\t\t";
			for(int relay_i=0; relay_i < node_num; relay_i++){
				if(path[relay_i] < 0)break;
				strtmp<<path[relay_i];
